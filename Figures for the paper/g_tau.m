...
for iter = 1:length(rs)
    ...
    figure(40+iter-1);clf;hold on;
    set(gcf,'Position', [100+300*(iter-1),380,250,200]);
    plot(taus,gs,'b','linewidth',2)
    plot(taus,Deltas/v_bar,'r:','linewidth',2)
    plot(taus,taus,'m--','linewidth',2)
    if iter ==1
        lh=legend({'$g$','$\Delta/\bar{v}$','$\tau$'},'interpreter','latex');
        set(lh,'Box','off')
    end
    xlabel('$\tau $','interpreter','latex')
    ylabel('$g$','interpreter','latex')
    title([subfig_ids{iter},' $\bar{v}/\bar{a}=$',num2str(rs(iter)),', $\kappa=$',num2str(kappa)],'interpreter','latex');
    saveas(gcf,['../../Figures/g_tau',num2str(iter)],'png');
end
