...
for iter = 1:4
    ...
    vs_bar = [1:20];
    rs_bar = [0.5:0.5:10];
    [v_mat,r_mat] = meshgrid(vs_bar,rs_bar);
    Delta_mat = zeros(size(v_mat));

    for i_mat = 1:size(v_mat,1)
        for j_mat = 1:size(v_mat,2)

            v_bar = v_mat(i_mat,j_mat);
            r = r_mat(i_mat,j_mat);
            a_bar = v_bar/r;
    ...
    surf(v_mat,r_mat,Delta_mat)
    ylabel('$\bar{v}/\bar{a}$','interpreter','latex')
...
%     theta = k*t/2;
%     phi = sqrt(max(4*k_mat-t_mat.^2.*k_mat.^2,0))/2;
