...
kappas = [0.1:0.1:2];
rs = [20,30];
kappas = [0.07:0.01:0.12];
[r_mat,k_mat] = meshgrid(rs,kappas);
tau_star_mat = zeros(size(k_mat));
g_star_mat = zeros(size(k_mat));
Delta_v_star_mat = zeros(size(k_mat));
...
for i_mat = 1:size(k_mat,1)
    for j_mat = 1:size(k_mat,2)
        ...
        taus = [0.1:.002:4];
        Deltas=zeros(size(taus));
        for i_vec = 1:length(taus)
        ...
        gs = Deltas/v_bar+taus;
        [g_star,i_star] = min(gs);
        tau_star_mat(i_mat,j_mat) = taus(i_star);
        g_star_mat(i_mat,j_mat) = g_star;
        Delta_v_star_mat(i_mat,j_mat) =Deltas(i_star)/v_bar;
    end
end
...
ang1 =90;
ang2 = 0;
f1 = figure(50);clf;hold on;
grid on;
set(gcf,'Position', [100,380,250,200]);
surf(r_mat,k_mat,g_star_mat);
 view(ang1,ang2)
...
f2=figure(51);clf;hold on;
grid on;
set(gcf,'Position', [400,380,250,200]);
surf(r_mat,k_mat,tau_star_mat);
view(ang1,ang2)
ylabel('$\kappa$','interpreter','latex')
xlabel('$\bar{v}/\bar{a}$','interpreter','latex')
...
f3 = figure(52);clf;hold on;
grid on;
set(gcf,'Position', [700,380,250,200]);
surf(r_mat,k_mat,Delta_v_star_mat);
view(ang1,ang2)
ylabel('$\kappa$','interpreter','latex')
xlabel('$\bar{v}/\bar{a}$','interpreter','latex')
zlabel('$\Delta^*/\bar{v}$','interpreter','latex')
title(['(c)'],'interpreter','latex');

f4=figure(53);clf;hold on;
grid on;
set(gcf,'Position', [1000,380,250,200]);
surf(r_mat,k_mat,k_mat.*tau_star_mat.^2);
view(ang1,ang2)
ylabel('$\kappa$','interpreter','latex')
xlabel('$\bar{v}/\bar{a}$','interpreter','latex')
zlabel('$\kappa\tau^{*2}$','interpreter','latex')
title(['(d)'],'interpreter','latex');

%  saveas(f1,['../../Figures/g_star'],'png');
%  saveas(f2,['../../Figures/tau_star'],'png');
%  saveas(f3,['../../Figures/Delta_star_v'],'png');
% saveas(f4,['../../Figures/kappa_tau_star_square'],'png');
