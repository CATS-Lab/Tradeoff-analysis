...
for iter = 1:4
    ...
    [k_mat,t_mat] = meshgrid(kappas,taus);
    phi_mat = sqrt(max(4*k_mat-t_mat.^2.*k_mat.^2,0))/2;
    Delta_mat = zeros(size(k_mat));

    for i_mat = 1:size(k_mat,1)
        for j_mat = 1:size(k_mat,2)

            kappa = k_mat(i_mat,j_mat);
            tau = t_mat(i_mat,j_mat);
            ...
            Delta_mat(i_mat,j_mat) =  Delta4_inf ;
        end
    end
    figure(23+iter);clf;hold on;
    grid on
    set(gcf,'Position', [100+300*(iter-1),380,300,250]);
    %set(gca,'InnerPosition',[0.1 0.2 .9 .9]);
    surf(k_mat,t_mat,Delta_mat/a_bar)
    ylabel('$\tau$','interpreter','latex')
    xlabel('$\kappa$','interpreter','latex')
    zlabel('$\Delta/\bar{a}$','interpreter','latex')
    %zlim([0,30]);
    title([subfig_ids{iter},' $\bar{v}/\bar{a}=$',num2str(v_bar)],'interpreter','latex');
    view(60,45)
    %saveas(gcf,['../../Figures/Delta_kappa_tau',num2str(iter)],'png');
